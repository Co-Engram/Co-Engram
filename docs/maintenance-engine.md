...
### Angle de-duplication, adversarial claim audit and handoff transfer (Phase3, 2026-08-18)

Phase3 closes the last three "same-source judgment" gaps:

- **P6 angle de-duplication**: two mechanical guarantees at plan finalization — any two engrams probe queries with character 2-gram Jaccard ≥ 0.7 count as the same angle re-read (the second is replaced with a keyword-slice variant; character-level, since Chinese has no word separators); the plan must contain an external-type item (web/mcp), mechanically appended when the LLM plan lacks one. Answer adjacency: Jaccard ≥ 0.65 against the previous run's final answer marks `answerRepeat` in the timeline (marker + audit, not blocking — "the previous conclusion still holds" is legitimate; v7 asks for a check, not a ban).
- **P7 adversarial claim audit**: when an L2 run submits its answer, an independent critic audits it adversarially (`extractClaims`: each claim judged `evidenced` = the answer explicitly grounds it in collected evidence / `downgraded` = unsupported assertion, speculation, deferred verification); **downgraded ratio > 30% → the run's insight proposals are all quarantined** ("weakly-supported answer", the same quarantine treatment as degraded but without changing the run's final state — resource closure and answer support are separate dimensions). Fail-open: skipped when no LLM is available (timeline records `claimsSkipped`; deployments without llmClient are not penalized). Claims and the ratio land in the timeline; the viewer's report shows a collapsible "Claim audit" section.
- **P8 handoff transfer**: at degraded finalization a critic generates the **next-run verification tasks** (`generateNextTasks`: 2-5 tasks grounded in the unclosed gaps and the partial answer; at least one external-source task mechanically guaranteed — appended by the engine if the LLM omits it); LLM failure degrades to the verbatim gap descriptions (Phase2's mechanical carry-over). `acquireThinking` stores `nextTasks ?? unclosedGaps` → the new plan consumes them with external-type routing (tasks matching external keywords become web plan items, otherwise engrams items) — the next run's agenda is no longer set by "the slacker themselves".

### Plan-first and engine-generated probes (Phase2, 2026-08-18)

Phase1's "self-declared list" compromise is closed out in Phase2 — **list generation right transfers**: at run startup (buildTask) the engine generates a **think plan** (requirement topology) and persists it (`run.plan`); the executor no longer authors the list:

- **Dual-source plan**: LLM (a critic-style single call generating 3-6 items from the question structure + seeds + past sessions: resource type / description / necessity / probe queries); a mechanical template falls back when no llmClient is available or generation fails (all five resource types, question-token probes; `planSource` audited). Unclosed gaps from a previous degraded run are **mechanically appended** to the new plan (cross-run relay, not LLM-dependent).
- **P5 anti-narrowing**: report requirements link plan items via `planItemId`; a dropped planned item is re-synthesized as an open gap by the engine; necessity downgrades are overridden by the plan; the executor may only **append** new requirements (subject to the gap budget — planned items do not count against the executor's budget, which exists to deter executor stalling, not to punish engine judgment).
- **P1 engine-generated probes**: engrams plan items carry ≥2 engine-generated probe queries the executor must run **verbatim** (closure checks match exactly — generation right controls the payload, closing the "performative probing" loophole); **auto-exemption**: when every probe variant ran and all returned empty (the engine witnesses `{hits:0}` in the call stream itself) the item auto-closes (probe-empty exemption) — the exemption right lives entirely engine-side; "the resource genuinely does not exist" is proven by the engine's own empty results, not by executor claims. Non-empty probes mean the resource exists and the item must close with real evidence.ids. Web probes are engine-generated (payload controlled) but execution is unobservable — closure claims display only; skills inventory is a mechanical call; logs/mcp have no probes.
- The headless (auto) path also carries the plan (rendered in the prompt) and has no repair loop — mine everything in one pass or finalize with gaps.
