...
Scheduling is **independent of REM cadence**: active entries run one single long session at the daily scheduled time (00:00 by default, per-entry configurable; due is checked on each light tick), and "run now" triggers one manually at any time. A cross-process in-flight lock (TTL 30 min) prevents double-counted rounds. Each run's prompt re-anchors the question and carries the full dream history (previous insights + accept/dismiss reasons) with a *deepen or pivot, do not repeat* instruction; insights too similar to history (Jaccard ≥ 0.65) are vetoed for that run (veto counts remain a diagnostic signal and no longer auto-pause anything). **Single-run semantics**: when a run finishes the entry moves to `suggested-resolve` awaiting the user's verdict — answering "did it answer your question?" with *yes* archives it (timeline preserved — the dream diary); with *no* the entry returns to active and the next scheduled anchor runs another round automatically (which again awaits the verdict). `pause` suspends auto-scheduling (manual runs are rejected; concluding is unaffected) and `delete` removes the entry (already-produced proposals and audit records are kept) — both available at any time and audited alongside rounds and conclusions.
