...
1. **本体（Invariant）**：不变的 Skill 标识与核心语义（skillId、initiation set、termination）
2. **投影（Projection）**：运行时动态计算的可变属性（utility、retention stage、acquisition stage）
3. **载体（Policy）**：可插拔的执行策略（Claude Skill、OpenClaw Skill、prompt、code、workflow）
...
- [工具参考](./tool-reference.md#skill-tools)（工具详细文档）
- [维护引擎](./maintenance-engine.md)（自动维护机制）
