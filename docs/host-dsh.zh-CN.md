...
Co-Engram 为 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 提供原生 Cordis 插件：38 个记忆工具全部注册为 dsh 原生工具（裸名形态,如 `engram_search`）,并注入 `memory:co-engram` system prompt 段——其 signals（topTags／技能清单／目录概览／待审候选数）在每次 prompt 组装时重新求值。
...
```
[co-engram] dsh plugin active: 12 engrams, 38 tools registered (host=dsh-plugin)
```
