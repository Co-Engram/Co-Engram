...
**Night thinking** is the flagship feature built on top: seed a question in chat (`incubation_create`), the viewer's **Night Lab** tab, or CLI; the agent runs one single long thinking session at the scheduled time (or immediately on demand), with full dream-history feedback and duplicate detection — and when it finishes, you decide: archive it, or authorize another run at the next anchor. Insights surface as proposals — plans, traces and external calls are fully transparent (the timeline includes per-insight reject reasons and the execution trace), web research is checked on by default in the viewer seeding form (uncheck to disable) and off by default when created via agent/tool, and accepted insights invite a resolve ritual ("did it answer your question?"). Entries can be paused (`incubation_pause`, suspends auto-scheduling) or deleted (`incubation_delete`, keeps already-produced proposals and audit records) at any time, and rounds/conclusions/pauses/deletions are all audited. On Claude Code, scheduled runs execute a headless `claude -p` L2 session with read-only tool grants; see [docs/maintenance-engine.md](./docs/maintenance-engine.md).

##### Night-thinking cadence and the daemon

- **Single run, anchored to a time of day**: at the scheduled time (00:00 local by default; the "Schedule" control on the entry card rewrites it per entry, `HH:mm`) one single long session runs, and when it finishes the entry awaits your verdict — answer "did it answer your question?" with *yes* to archive it, or *not yet* to return it to active so the next scheduled anchor runs another round automatically (which again awaits the verdict); "Run now" triggers a session at any time.
- **Missed runs catch up automatically**: if no process is alive at the anchor time (sessions closed, daemon exited), the run is not lost — the first maintenance tick of the next live process (the next session, or after the daemon starts) catches up due runs. The "Scheduler" indicator in the Night Lab header shows live whether a process is alive (the lock-holding session or the daemon; a heartbeat within 90 s counts as running).
- **On-time execution**: to run exactly at the anchor time, a process must be alive then — Claude Code runs in single-daemon mode by default (one shared long-lived daemon for all sessions, auto-exits after 30 min idle, disable via `CO_ENGRAM_DAEMON`), so keeping one Claude Code session open is enough; OpenClaw has no such daemon and follows the plugin process.
- **Interim drafts and concluding**: each run automatically produces an interim answer draft (click the entry card to read it); once it has thought enough, "Conclude" synthesizes the final answer at any time.
- **Per-run resources**: each run draws on the full memory graph + activity log + skill library. Web research: the viewer seeding form ships checked (on by default, uncheck to disable), while agent/tool creation defaults to off and requires explicit opt-in; when enabled, only the question and digest-level content are sent to the search engine.
- **Many entries at a glance**: the active list shows the first 5 cards by default (the rest folded) with question-text filtering; pause/resume/delete live on each entry card.
- **Behavior change**: a newly created entry no longer runs at creation — the first run waits for the first anchor time, or a manual "Run now".
