...
**夜思**是构建其上的核心差异化功能:在对话(`incubation_create`)、viewer「夜思实验室」页或 CLI 播种一个问题,Agent 在排程时刻替你执行一次单次长任务(也可立即触发),带完整梦境史回灌与重复检测;跑完待你裁决 —— 归档,或授权下个锚点再跑一轮。洞察以提案呈现 —— 计划、轨迹与外部调用完全透明(时间线含逐条拒因与执行轨迹),联网调研在 viewer 播种表单默认勾选开启(可取消)、经 agent/工具创建默认关闭,accept 洞察后有 resolve 仪式(「是否回答了你的问题?」)。条目可随时暂停自动排程(`incubation_pause`)或删除(`incubation_delete`,已产出的提案与审计保留),轮次/收束/暂停/删除均纳入审计。Claude Code 端定时执行以只读工具授权运行无头 `claude -p` L2 会话;详见 [docs/maintenance-engine.zh-CN.md](./docs/maintenance-engine.zh-CN.md)。

##### 夜思节奏与 daemon

- **单次执行,锚点时刻制**:排程时刻(默认每日凌晨 00:00 本地时区,条目卡片「排程」可按条目改写 `HH:mm`)到点执行一次单次长任务,跑完进入待裁决状态 —— 回答「是否回答了你的问题」,是则归档;选「还没有」则条目回 active,下个排程锚点自动再跑一轮(跑完再次待裁决),也可随时手动「立即夜思」。
- **错过自动补跑**:排程时刻没有存活进程(会话已关、daemon 已退出)时该次执行不丢失 —— 下一个存活进程(下次会话,或 daemon 启动后)的第一个维护 tick 会补跑到期的执行。夜思页头的「调度器」状态实时显示当前是否有进程存活(持有锁的会话或 daemon,心跳 90 秒内即视为运行中)。
- **准点执行**:要严格在锚点时刻执行,排程时刻需有存活进程 —— Claude Code 默认单 daemon 模式(所有会话共享一个常驻守护进程,空闲 30 分钟自动退出,`CO_ENGRAM_DAEMON` 可关),保持一个 Claude Code 会话开着即可;OpenClaw 无此 daemon,以插件进程存活为准。
- **阶段草稿与收束**:每次执行自动生成一份阶段性回答草稿(条目卡片点击查看);觉得想清楚了,随时「收束出结论」生成最终回答。
- **执行资源**:每次夜思使用全部记忆图谱 + 行为日志 + 技能库;联网调研:viewer 播种表单默认勾选开启(可取消);经 agent/工具创建默认关闭,需显式开启。开启后仅问题与摘要级内容会发送至搜索引擎。
- **多条目管理**:活跃条目默认只展开前 5 张卡片(其余折叠),支持按问题文本过滤;暂停/恢复/删除按钮在条目卡片上。
- **行为变更**:新建条目不再「创建即跑」—— 首次执行等第一个排程时刻到来,或手动「立即夜思」触发。
