...
**夜思**是构建其上的核心差异化功能:在对话(`incubation_create`)、viewer「夜思实验室」页或 CLI 播种一个问题,Agent 每夜替你想一轮(也可立即触发),带完整梦境史回灌与重复检测。洞察以提案呈现 —— 计划、轨迹与外部调用完全透明,联网调研在 viewer 播种表单默认勾选开启(可取消)、经 agent/工具创建默认关闭,accept 洞察后有 resolve 仪式(「是否回答了你的问题?」)。Claude Code 端定时轮次以只读工具授权运行无头 `claude -p` L2 会话;详见 [docs/maintenance-engine.zh-CN.md](./docs/maintenance-engine.zh-CN.md)。

##### 夜思节奏与 daemon

- **每天一轮,锚点时刻制**:每条目每天跑一轮,默认锚点在凌晨 00:00(本地时区);条目卡片上的「排程」可按条目改写(`HH:mm`)。
- **错过自动补跑**:排程时刻没有存活进程(会话已关、daemon 已退出)时该轮不丢失 —— 下一个存活进程(下次会话,或 daemon 启动后)的第一个维护 tick 会补跑到期的轮次。夜思页头的「调度器」状态实时显示当前是否有进程存活(持有锁的会话或 daemon,心跳 90 秒内即视为运行中)。
- **准点执行**:要严格在锚点时刻执行,排程时刻需有存活进程 —— Claude Code 默认单 daemon 模式(所有会话共享一个常驻守护进程,空闲 30 分钟自动退出,`CO_ENGRAM_DAEMON` 可关),保持一个 Claude Code 会话开着即可;OpenClaw 无此 daemon,以插件进程存活为准。也可随时手动「立即夜思」触发一轮。
- **阶段草稿与收束**:每轮自动生成一份阶段性回答草稿(条目卡片点击查看);觉得想清楚了,随时「收束出结论」生成最终回答。
- **轮次资源**:每轮夜思使用全部记忆图谱 + 行为日志 + 技能库;联网调研:viewer 播种表单默认勾选开启(可取消);经 agent/工具创建默认关闭,需显式开启。开启后仅问题与摘要级内容会发送至搜索引擎。
- **行为变更**:新建条目不再「创建即跑」—— 首轮等第一个排程时刻到来,或手动「立即夜思」触发。
