...
describe("profile contract: claude-code-mcp ≡ openclaw-plugin", () => {
  ...
  it("actual counts match observed real values (12/40/48, 含 incubation × 7)", () => {
    ...
    // 夜思改版(2026-08-16):incubation × 5 → × 7(新增 conclude/update)
    expect(CC.PROFILE_TOOL_SETS.minimal.size).toBe(12);
    expect(CC.PROFILE_TOOL_SETS.standard.size).toBe(40);
    expect(CC.PROFILE_TOOL_SETS.full.size).toBe(48);
  });
