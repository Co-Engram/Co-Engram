...
describe("ToolRegistry", () => {
  it("列出所有工具（48 个：P0 12 + P1 5 + P2 2 + P3 2 + M1 proposal 3 + AI-8 batch proposal 2 + doctor/list_paths 2 + synthesize 1 + engram_sync 1 + audit_query 1 + skill CRUD 5 + skill invoke 1 + skill compose 3 + skill related_engram 3 + incubation 7）", () => {
    const reg = createToolRegistry();
    expect(reg.list().length).toBe(48);
  });
  ...
  it("按命名空间筛选", () => {
    ...
    expect(reg.listByNamespace("skill").length).toBe(12); // skill_create/list/get/update/delete/invoke + skill_compose_add/remove/list + skill_related_engram_add/remove/list
