...
RETURNS: Skill metadata (name, description, template kind, parameters).`,
  ...
  // ===== Skill retirement proposals (proposal.skillRetire.*, generated by light cycles, shown in proposal center) =====
  "proposal.skillRetire.title":
    "Retire skill ${skillId} (${days} days zero-use)",
  "proposal.skillRetire.content":
    "Registered on ${createdAt}, this skill has never been invoked and has been zero-use for ${days} days (retentionStage=${stage}). Proposed retirement: remove it from skill_list and the system-prompt injection catalog to free attention budget; the source SKILL.md and imprint are kept (skill_get still works; a single real usage revives the skill and auto-withdraws this proposal). accept = retire; dismiss = keep and reset the clock.",

  "tool.skill_invoke.agent": `Report a Skill usage result and update utility + retention via Rescorla-Wagner. **This tool only records usage, does not execute the skill** — the host executes; agents call this after using a skill so its procedural imprint evolves with usage. A forgotten or retired skill auto-revives on the first usage report (real use is the strongest reactivation signal; review SKILL.md if long unused).
...
RETURNS: Created skill record with utility=0.5, acquisitionStage=draft, retentionStage=active.`,
  "tool.skill_list.agent": `List all skills with optional filtering by acquisition/retention stage; retired skills are excluded by default — pass includeRetired: true to include them (audit use).
...
Invariant: relatedIds derived from synapses (both directions).`,
  ...
  "viewer.contemplation.status.repairing": "Repairing gaps",
  ...
  "viewer.stats.weeklyRetrievals": "↑${n}/wk",
  "viewer.stats.weeklyInvocations": "↑${n}/wk",
  ...
  "viewer.proposals.sourceLine.remInsight": "Night-thinking insight",
  "viewer.proposals.sourceLine.remInsightContemplation": "Contemplation insight",
  "viewer.proposals.sourceLine.remPattern": "Pattern distillation",
  "viewer.proposals.sourceLine.remSynapse": "Synapse suggestion",
  "viewer.proposals.sourceLine.remTagRefresh": "Tag refresh",
  "viewer.proposals.sourceLine.remVerification": "Liveness check",
  ...
  "viewer.proposals.why.sourceLabel.remInsight": "Night-thinking insight",
  "viewer.proposals.why.sourceLabel.remInsightContemplation": "Contemplation insight",
  ...
  "viewer.maintenance.sleep.decayDeep": "Merge cleanup",
  ...
  "viewer.maintenance.sleep.decayDeepSub": "deep · duplicate merges (card shows from → to); stale archive/forget in the next cell",
  ...
  "viewer.maintenance.sleep.nextOverdue": "Next: ${stage} is overdue — catch-up runs when the engine is idle",
  ...
  "viewer.help.skill.lifecycle":
    "<strong>Lifecycle</strong>: <code>detection (proposal) → accept (persist) → invoke (use) → decay → forgotten</code>. <code>detection</code>: scan <code>SKILL.md</code> directories and generate candidate proposals. <code>accept</code>: create Skill entity with acquisitionStage=draft, retentionStage=active. <code>invoke</code>: record usage count, update utility via <strong>Rescorla-Wagner rule</strong> (up on success, down on failure). <code>decay</code>: light maintenance cycles recalculate retention, dropping active → aging/stale over time (never-used skills age from registration time). <code>forgotten</code>: long-unused skills auto-forget and leave the injected skill catalog; a single real use revives them (relearning). <code>retired</code>: zero-use skills in stale/forgotten stage get a <strong>retirement proposal</strong> from light cycles (visible in the proposal center); accepting removes the skill from skill_list and the injection catalog (source SKILL.md and imprint are kept), dismissing keeps it and resets the clock; any real usage revives it and auto-withdraws the proposal.",
  ...
  "viewer.help.skill.retention":
    "<strong>retention</strong>: reflects memory strength for the skill, decaying over time along the forgetting curve (anchored to last use for used skills, or to registration time for never-used skills), 4 levels: <code>active (strong)</code>: recently used frequently, high retention. <code>aging (weakening)</code>: starting to forget but still recallable. <code>stale (weak)</code>: low retention, may need relearning. <code>forgotten (gone)</code>: removed from default skill pool; <strong>a single real use revives it automatically</strong> (relearning — the next agent trigger just calls and reports it), manual restore on the web UI also works. Separately, <strong>retired</strong> is a user-adjudicated state (accepting a retirement proposal), not a decay projection: the skill leaves skill_list and the injection catalog, remains readable via skill_get, and revives on use.",
