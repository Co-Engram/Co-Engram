...
Invariant: relatedIds derived from synapses (both directions).`,
  ...
  "viewer.incubations.l2BudgetNotice": "Night thinking calls an external LLM (may consume significant tokens). The L2 session process (plan/trace) is fully transparent; web research is on by default and can be turned off per entry.",
  ...
  "viewer.stats.effectiveRateTip": "Effective = retrievals followed by a reinforcement feedback within the observation window. Retrieves the agent never reported feedback for count as unclosed and lower the ratio — it measures feedback-loop closure, not retrieval quality.",
  ...
  "viewer.graph.reading.hint": "Node size = importance<br>red dashed = the two memories contradict each other",
