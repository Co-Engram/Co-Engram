...
Invariant: relatedIds derived from synapses (both directions).`,
  ...
  "viewer.help.contemplation.closure": "Closure check: at run startup the engine first generates a <strong>think plan</strong> (requirement list with probe queries — the executor runs probes verbatim and may only append, never drop or downgrade). Reports are validated against the plan: dropped or downgraded items are restored by the engine, and claiming \"already read\" without real calls sends the run back for repair (in-chat agents fix and re-report). Items whose probes all ran empty are auto-exempted on engine-witnessed evidence. Persistently open gaps or an exhausted repair budget (default 6 rounds) finalizes the run as <strong>degraded</strong>: the card shows a degraded badge with the unclosed requirements, and its insight proposals go to the proposals tab's <strong>quarantine zone</strong> — excluded from the default approval queue (adjudicate them from the \"All\" view).",
