...
不变量:relatedIds 派生自 synapse(双向)。`,
  ...
  "viewer.incubations.l2BudgetNotice": "夜思会调用外部 LLM（可能消耗较多 token）。L2 会话过程（计划/轨迹）全部透明可查；下方联网复选框默认勾选 —— 取消勾选即完全离线；开启后仅问题与摘要级内容会发送至搜索引擎。",
  ...
  "viewer.incubations.explainer.what": "夜思是全资源盘点式多夜孵化：依次调用全部记忆图谱、行为日志、你的技能库（联网需按条目开启）围绕问题逐夜深思。",
  "viewer.incubations.explainer.how": "每夜一轮：产出洞察草稿，经引用校验、重复检测、独立评审三道闸后成为提案；你 accept/dismiss 的结果会回灌下一轮，逐步收敛。",
  "viewer.incubations.explainer.gain": "每轮自动生成阶段性回答草稿（点击卡片查看）；你认为够了可随时「收束」出最终回答，accept 的洞察进入记忆库。",
  ...
  "viewer.incubations.sownTip": "已播种。首轮预计 ${time}（或点「立即夜思」）；此后每天 ${schedule} 一轮；每轮生成阶段草稿，随时可收束。",
